...
void BenchmarkViewUpdateHeuristic::doUpdate(const QModelIndex & topLeft, const QModelIndex & bottomRight, const QVector<int> &roles)
{
    Q_UNUSED(roles);
...
void BenchmarkViewMashup::doUpdate(const QModelIndex & topLeft, const QModelIndex & bottomRight, const QVector<int> &roles)
{
    Q_UNUSED(roles);
    ...
    if (topLeft == bottomRight && topLeft.isValid()) {
        ...
        if (isVisible() && !d->delayedPendingLayout)  {
            ...
            else if(cornerRectViewPort.width() * cornerRectViewPort.height() * 8 > viewportRect.width() * viewportRect.height()){
                const QModelIndex parent = topLeft.parent();
                QRect dirty(cornerRect);
                const int maxRow = bottomRight.row();
                for (int i = topLeft.row(); i < maxRow; ++i)  {
